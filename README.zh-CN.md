...
[**BookAlign**](https://github.com/ArcadiaLin/bookalign) 从另一侧解同一个配对
问题：多语 embedding（LaBSE）加动态规划，比的是段落的**意思**而不是长度。这正好
够得到上表够不到的那种情况——配错了却毫不含糊，长度恰好对得上，纯长度模型会自信地
配错。它主要面向小说，尤其是日语原著配中文译本，走的是先复查再对齐的分步流程，而
不是一把跑到底。

两者互补：这边不用下模型、哪儿都能跑；那边会读文字，代价是一个本地 LaBSE 模型，
实际上还需要一张显卡。如果你手上两个版本差异大到光靠长度就会配错，从那边开始。
